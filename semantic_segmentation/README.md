...
### UrbanMamba
A dual-branch architecture for urban semantic segmentation that combines MambaVision with NSST (Non-Subsampled Shearlet Transform) for enhanced texture and edge detection. See [`urban_mamba/README.md`](urban_mamba/README.md) for details.

**Key Features:**
- Dual-branch architecture (RGB spatial + NSST XLET)
- 87-channel frequency-domain feature extraction
- Adaptive multi-scale fusion with VMamba blocks
- Urban Context Decoder for sharp boundary preservation
